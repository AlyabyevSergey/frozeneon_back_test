...
class User_model extends Emerald_model {
    ...
    public function add_money(float $sum): bool
    {
        ...
        $wb_now = (float) self::get_wallet_balance();
        self::set_wallet_balance($wb_now + $sum);
        
        $wb_all = (float) self::get_wallet_total_refilled();
        self::set_wallet_total_refilled($wb_all + $sum);
    ...
    public function remove_money(float $sum): bool
    {
        ...
        $wtw_now = (float) self::get_wallet_total_withdrawn();
        self::set_wallet_total_withdrawn($wtw_now + $sum);
    ...
    public static function find_user_by_id(int $user_id): User_model
    {
        // TODO: task 1, аутентификация
        return static::transform_one(App::get_s()->from(self::CLASS_TABLE)->where(['id' => $user_id])->one());
    }
